...
export default function AdminFooter() {
    return (
        <footer className="AdminFooter bg-[#1D1D1D] h-[100px] w-full">
            <div className="max-w-[1280px] h-full mx-auto px-4 flex items-center gap-8">
                ...
                <div className="flex flex-col ml-[40px] items-start justify-center">
                    <div className="flex gap-x-2 mb-2">
                        {['Приказы', 'ELO баллы', 'Поддержка', 'Судейский корпус', 'Пользовательское соглашение'].map((text, i) => (
                            <span
                                key={i}
                                className="text-[#757575] mr-[20px] hover:text-[#A1A1A1] cursor-pointer"
                            >
                                {text}
                            </span>
                        ))}
